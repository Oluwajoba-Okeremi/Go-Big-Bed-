...
final class SleepSessionStore: ObservableObject {
    ...
    /// Provide (start, end). Caller should persist to HealthKit.
    ...
    func bootstrap() {
        ...
        if let saved = ud.object(forKey: kStartDate) as? Date,
           ud.bool(forKey: kIsRunning) == true {
            ...
            // If the app was killed and returned AFTER the intended cutoff,
            // rescue the session and notify for saving.
    ...
    /// Call this from the Stop button action.
    /// It ends the session AND triggers onAutoEnded so callers always save.
    func endAndNotify(reason: String = "manual") {
        guard let result = endNow() else { return }
        // Respect minSeconds for *points*, but ALWAYS notify to allow Health write.
        // Upstream can choose to ignore points if too short.
    ...
    func appWillEnterForeground() {
        ...
        // If we crossed cutoff while backgrounded, end and notify immediately.
        if shouldAutoEnd(now: Date(), start: s) {
    ...
    // MARK: - Cutoff Logic
    /// Friday/Saturday *nights* have **no cutoff** (matches prior requirement).
    /// We consider the "night" to belong to the day whose noon boundary contains the session.
    private func nightWeekday(for start: Date) -> Int {
        let cal = Calendar.current
        // Noon anchor like Health: if start < noon, the "night" is the previous day.
        let noon = cal.date(bySettingHour: 12, minute: 0, second: 0, of: start)!
        let nightDate = (start < noon) ? cal.date(byAdding: .day, value: -1, to: noon)! : noon
    ...
    private func cutoffDate(for start: Date) -> Date {
        if isNoCutoffNight(start) {
            return .distantFuture // No cutoff on Fri/Sat nights
        }
        let cal = Calendar.current
    ...
    private func startTicker() {
        ...
        ticker = Timer.publish(every: tickInterval, on: .main, in: .common)
            ...
            .sink { [weak self] _ in
                ...
                if self.shouldAutoEnd(now: now, start: s) {
                    // Auto end at cutoff and notify save.
    ...
    private func startMotionMonitoring() {
        motionGuard.startMonitoring(
            ...
            onViolation: { [weak self] in
                ...
                // Treat a violation as a definite end and notify for saving.
