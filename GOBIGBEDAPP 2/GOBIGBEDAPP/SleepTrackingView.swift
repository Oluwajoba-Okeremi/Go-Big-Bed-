...
struct SleepTrackingView: View {
    ...
    private var idleView: some View {
        VStack(spacing: 12) {
            ...
            Text("**Session can run anytime between 8pm – 12pm(If the button is greyed during these hours, change to a different tab then change back)**")
                .font(.subheadline)
                .foregroundColor(Theme.textMuted)
                .frame(maxWidth: .infinity, alignment: .leading)

            Text("**Do Not Move Your Phone after starting, Timer will Stop!**")
                .font(.subheadline)
                .foregroundColor(Theme.textMuted)
                .frame(maxWidth: .infinity, alignment: .leading)
        }
